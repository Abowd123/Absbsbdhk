...
async def get_rank(id, cid) -> str:
   ...
   if await r.get(f'{cid}:rankGOWNER:{id}{Dev_Zaid}'):
      if await r.get(f'{cid}:RankGowner:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankGowner:{Dev_Zaid}')
      return 'المالك الاساسي'
   if await r.get(f'{cid}:rankOWNER:{id}{Dev_Zaid}'):
      if await r.get(f'{cid}:RankOwner:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankOwner:{Dev_Zaid}')
      return 'المالك'
   if await r.get(f'{cid}:rankMOD:{id}{Dev_Zaid}'):
      if await r.get(f'{cid}:RankMod:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankMod:{Dev_Zaid}')
      return 'المدير'
   if await r.get(f'{cid}:rankADMIN:{id}{Dev_Zaid}'):
      if await r.get(f'{cid}:RankAdm:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankAdm:{Dev_Zaid}')
      return 'ادمن'
   if await r.get(f'{cid}:rankPRE:{id}{Dev_Zaid}'):
      if await r.get(f'{cid}:RankPre:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankPre:{Dev_Zaid}')
      return 'مميز'
   else:
      if await r.get(f'{cid}:RankMem:{Dev_Zaid}'):
         return await r.get(f'{cid}:RankMem:{Dev_Zaid}')
      return 'عضو'
