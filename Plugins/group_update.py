...
async def global_filter(c,m):
   ...
   if await r.get(f'{m.chat.id}:lock_global:{Dev_Zaid}'):  return 
   ...
   if await r.get(f'{m.chat.id}:addFilterG:{m.from_user.id}{Dev_Zaid}'):  return
   if await r.get(f'{m.chat.id}:delFilterG:{m.from_user.id}{Dev_Zaid}'):  return 
   if await r.get(f'{m.chat.id}:addFilter2GG:{m.from_user.id}{Dev_Zaid}'):  return 
   text = m.text
   name = await r.get(f'{Dev_Zaid}:BotName') or 'رعد'
   if text.startswith(f'{name} '):
      text = text.replace(f'{name} ','')
   if await r.get(f'{text}:filter:{Dev_Zaid}'):
     get = await r.get(f'{text}:filter:{Dev_Zaid}')
   ...
   if text == 'المطور':
     id = int(await r.get(f'{Dev_Zaid}botowner'))
...
async def get_filter(c,m):
   if not await r.get(f'{m.chat.id}:enable:{Dev_Zaid}'):  return
   if await r.get(f'{m.chat.id}:mute:{Dev_Zaid}') and not await admin_pls(m.from_user.id,m.chat.id):  return 
   if await r.get(f'{m.chat.id}:addFilter:{m.from_user.id}{Dev_Zaid}'):  return
   if await r.get(f'{m.chat.id}:delFilter:{m.from_user.id}{Dev_Zaid}'):  return 
   if await r.get(f'{m.chat.id}:addFilter2:{m.from_user.id}{Dev_Zaid}'):  return 
   if await r.get(f'{m.chat.id}:lock_filter:{Dev_Zaid}'):  return 
   if await r.get(f'{m.from_user.id}:mute:{m.chat.id}{Dev_Zaid}'):  return 
   if await r.get(f'{m.from_user.id}:mute:{Dev_Zaid}'):  return 
   if await r.get(f'{m.chat.id}:addCustom:{m.from_user.id}{Dev_Zaid}'):  return 
   if await r.get(f'{m.chat.id}addCustomG:{m.from_user.id}{Dev_Zaid}'):  return 
   ...
   if text.startswith(f'{name} '):
      text = text.replace(f'{name} ','')
   if await r.get(f'{text}:filter:{Dev_Zaid}{m.chat.id}'):
     get = await r.get(f'{text}:filter:{Dev_Zaid}{m.chat.id}')
     type = re.search(r'type=([^&]+)', get).group(1)
     userID = str(m.from_user.id)
     userNAME = str(m.from_user.first_name)
     userUSERNAME = "@"+m.from_user.username if m.from_user.username else "مافي يوزر"
   ...
   if await r.get(f'{text}:filterMEM:{Dev_Zaid}{m.chat.id}') and not await r.get(f'{m.chat.id}:lock_filterMEM:{Dev_Zaid}'):
     id = int(await r.get(f'{text}:filterMEM:{Dev_Zaid}{m.chat.id}'))
     get = await c.get_chat(id)
     ...
     if not get.photo:
       ...
       if await r.get(f"{hash}:{id}"):
         return await m.reply_animation(await r.get(f"{hash}:{id}"), caption=cap, reply_markup=reply_markup) 
       video = photo.video_sizes[0] if photo.video_sizes else None
       if video:
         file = BytesIO()
         for byte in c.stream_media(
           ...
           return await r.set(f"{hash}:{id}", a.animation.file_id, ex=120)
       else:
         for photo in c.get_chat_photos(id, limit=1):
...
async def get_rngp(c,m):
   if not await r.get(f'{m.chat.id}:enable:{Dev_Zaid}'):  return
   if await r.get(f'{m.chat.id}:mute:{Dev_Zaid}') and not await admin_pls(m.from_user.id,m.chat.id):  return
   if await r.get(f'{m.chat.id}:lock_global:{Dev_Zaid}'):  return 
   
   if m.from_user:
     if await r.get(f'{m.chat.id}:addFilterRG:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:delFilterRG:{m.from_user.id}{Dev_Zaid}'):  return 
     if await r.get(f'{m.from_user.id}:mute:{m.chat.id}{Dev_Zaid}'):  return
     if await r.get(f'{m.from_user.id}:mute:{Dev_Zaid}'):  return 
   ...
   userUSERNAME = "@"+m.from_user.username if m.from_user.username else "مافي يوزر"
   userMENTION = m.from_user.mention(userNAME[:25])
   if await r.get(f'{text}:randomFilter:{Dev_Zaid}'):
     if await r.smembers(f'{text}:randomfilter:{Dev_Zaid}'):
       list = await r.smembers(f'{text}:randomfilter:{Dev_Zaid}')
       return await m.reply(random.sample(list,1)[0].replace('{اسم_البوت}',name).replace("<USER_ID>",userID).replace("<USER_NAME>",userNAME).replace("<USER_USERNAME>",userUSERNAME).replace("<USER_MENTION>",userMENTION), disable_web_page_preview=True)
...
async def get_rn_filter(c,m):
   ...
   if m.from_user:
     ...
     if await r.get(f'{m.chat.id}:delFilter:{m.from_user.id}{Dev_Zaid}'):  return 
     if await r.get(f'{m.chat.id}:addFilter2:{m.from_user.id}{Dev_Zaid}'):  return 
     if await r.get(f'{m.chat.id}:delFilterR:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:addFilterR:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:addFilterR2:{m.from_user.id}{Dev_Zaid}'):  return 
   text = m.text
   name = await r.get(f'{Dev_Zaid}:BotName') or 'رعد'
   userID = str(m.from_user.id)
   userNAME = str(m.from_user.first_name)
   ...
   if text.startswith(f'{name} '):
      text = text.replace(f'{name} ','')
   if await r.get(f'{text}:randomFilter:{m.chat.id}{Dev_Zaid}'):
       list = await r.smembers(f'{text}:randomfilter:{m.chat.id}{Dev_Zaid}')
       return await m.reply(random.sample(list,1)[0].replace("<USER_ID>",userID).replace("<USER_NAME>",userNAME).replace("<USER_USERNAME>",userUSERNAME).replace("<USER_MENTION>",userMENTION), disable_web_page_preview=True)
...
async def kick_from_gp(c,m):
   if m.left_chat_member.id == int(Dev_Zaid):
        ...
        await r.delete(f'{m.chat.id}:enable:{Dev_Zaid}')
        if await r.smembers(f'enablelist:{Dev_Zaid}'):
          text += f'\n{k} عدد المجموعات الآن : {len(await r.smembers(f"enablelist:{Dev_Zaid}"))}\n'
        text += f'\n{k} تم مسح جميع بيانات المجموعة'
        text += '\n\n☆'
        if await r.get(f'DevGroup:{Dev_Zaid}'):
          await c.send_message(int(await r.get(f'DevGroup:{Dev_Zaid}')),text,disable_web_page_preview=True)
        else:
    ...
    if m.new_chat_member.status == ChatMemberStatus.MEMBER:
       if m.new_chat_member.user.id == c.me.id:
         if await r.get(f'{m.chat.id}:enable:{Dev_Zaid}'):
             ...
             text += '\n\n☆'
             if await r.get(f'DevGroup:{Dev_Zaid}'):
                   await c.send_message(int(await r.get(f'DevGroup:{Dev_Zaid}')),text)
             else:
               for dev in await get_devs_br():
                 try:
                    await c.send_message(int(dev), text, disable_web_page_preview=True)
       ...
       if m.new_chat_member.user.id == c.me.id:
          ...
          if not await r.get(f'{m.chat.id}:enable:{Dev_Zaid}'):
             ...
             if priv.can_manage_chat and priv.can_delete_messages and priv.can_restrict_members and priv.can_pin_messages and priv.can_invite_users:
                ...
                for member in await m.chat.get_members(filter=ChatMembersFilter.ADMINISTRATORS):
                   if not member.user.is_bot and not member.user.is_deleted:
                      if member.status == ChatMemberStatus.OWNER:
                         await r.set(f'{m.chat.id}:rankGOWNER:{member.user.id}{Dev_Zaid}', 1)
                         await r.sadd(f'{m.chat.id}:listGOWNER:{Dev_Zaid}', member.user.id)
                         await r.sadd(f'{member.user.id}:groups', m.chat.id)
                      if member.status == ChatMemberStatus.ADMINISTRATOR:
                         await r.set(f'{m.chat.id}:rankADMIN:{member.user.id}{Dev_Zaid}', 1)
                         await r.sadd(f'{m.chat.id}:listADMIN:{Dev_Zaid}', member.user.id)
                ...
                text += '\n\n☆'
                if await r.get(f'DevGroup:{Dev_Zaid}'):
                   await c.send_message(int(await r.get(f'DevGroup:{Dev_Zaid}')),text,reply_markup=reply_markup,disable_web_page_preview=True)
                else:
                  for dev in await get_devs_br():
                    try:
