...
async def private_func(c,m,k):
  ...
  text = m.text
  #await r.set(f'DevGroup:{Dev_Zaid}'
...
''', reply_markup=InlineKeyboardMarkup ([
  ...
  [InlineKeyboardButton (f'تحديثات {name} 🍻', url=f'https://t.me/{channel}')]
     ...
     if not await r.sismember(f'{Dev_Zaid}:UsersList',m.from_user.id):
       await r.sadd(f'{Dev_Zaid}:UsersList',m.from_user.id)
...
☆ عدد المستخدمين صار {}
'''.format(m.from_user.mention,m.from_user.id,username,len(await r.smembers(f'{Dev_Zaid}:UsersList')))
       ...
       if await r.get(f'DevGroup:{Dev_Zaid}'):
          await c.send_message(
          int(await r.get(f'DevGroup:{Dev_Zaid}')),
...
async def sudosCommandsHandler(c,m):
    k = await r.get(f'{Dev_Zaid}:botkey')
    channel = await r.get(f'{Dev_Zaid}:BotChannel') or 'yqyqy66'
    await SudosCommandsFunc(c,m,k,r,channel)

async def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if (await r.get(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Zaid}') or await r.get(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}')) and text == 'الغاء':
       await m.reply(quote=True,text=f'{k} من عيوني لغيت كل شي')
       await r.delete(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Zaid}')
       await r.delete(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Zaid}')
       await r.delete(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Zaid}')
       await r.delete(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Zaid}')
       return await r.delete(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}')

   if await r.get(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Zaid}') and await dev2_pls(m.from_user.id,m.chat.id):
      await r.delete(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Zaid}')
      await r.set(f'{Dev_Zaid}:BotName',m.text)
      return await m.reply(quote=True,text=f'{k} ابشر عيني المطور غيرت اسمي لـ {m.text}')
   
   if await r.get(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Zaid}') and await dev2_pls(m.from_user.id,m.chat.id):
      await r.delete(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Zaid}')
      await r.set(f'{Dev_Zaid}:BotChannel',m.text.replace('@',''))
      return await m.reply(quote=True,text=f'{k} ابشر عيني غيرت قناة السورس لـ {m.text}')
   
   if await r.get(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Zaid}') and await dev2_pls(m.from_user.id,m.chat.id):
      await r.delete(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Zaid}')
      await r.set(f'{Dev_Zaid}:botkey',m.text)
      return await m.reply(quote=True,text=f'{k} ابشر عيني غيرت رمز السورس لـ {m.text}')
      
   if await r.get(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Zaid}') and await devp_pls(m.from_user.id,m.chat.id):
      await r.delete(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Zaid}')
      try:
        id = int(m.text)
      except Exception:
        return await m.reply(quote=True,text=f'{k} الايدي غلط!')
      await r.set(f'DevGroup:{Dev_Zaid}', int(m.text))
      return await m.reply(quote=True,text=f'{k} ابشر عيني قروب المطور لـ {m.text}')
   
   if await r.get(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}') and await devp_pls(m.from_user.id,m.chat.id):
      await r.delete(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}')
      try:
        get = await c.get_chat(m.text.replace('@',''))
      except Exception:
        return await m.reply(quote=True,text=f'{k} اليوزر غلط!')
      await r.set(f'{Dev_Zaid}botowner', get.id)
   ...
   if text == 'الاحصائيات':
      if not await dev2_pls(m.from_user.id,m.chat.id):
         return 
      if not await r.smembers(f'{Dev_Zaid}:UsersList'):
         users = 0
      else:
         users = len(await r.smembers(f'{Dev_Zaid}:UsersList'))
      if not await r.smembers(f'enablelist:{Dev_Zaid}'):
         chats = 0
      else:
         chats = len(await r.smembers(f'enablelist:{Dev_Zaid}'))
      return await m.reply(quote=True,text=f'{k} هلا بك مطوري\n{k} المستخدمين ~ {users}\n{k} المجموعات ~ {chats}')
   
   if text == 'تفعيل البوت الخدمي':
      if not await dev2_pls(m.from_user.id,m.chat.id):
         return 
      if not await r.get(f'DisableBot:{Dev_Zaid}'):
         return await m.reply(quote=True,text=f'{k} البوت الخدمي مفعل من قبل')
      else:
         await r.delete(f'DisableBot:{Dev_Zaid}')
   ...
   if text == 'المستخدمين المحظورين' or text == 'المحظورين عام':
     if not await dev_pls(m.from_user.id, m.chat.id):
        ...
        if not await r.smembers(f'listGBAN:{Dev_Zaid}'):
           ...
           count = 1
           for user in await r.smembers(f'listGBAN:{Dev_Zaid}'):
               try:
   ...
   if text == 'المجموعات المحظورة':
     if not await dev2_pls(m.from_user.id, m.chat.id):
        ...
        if not await r.smembers(f':BannedChats:{Dev_Zaid}'):
           return await m.reply(quote=True,text=f'{k} مافي قروب محظور عام')
        else:
           text = 'المجموعات المحظورة عام:\n'
           count = 1
           for user in await r.smembers(f':BannedChats:{Dev_Zaid}'):
               text += f'{count}) {user}\n'
               count += 1
           return await m.reply(quote=True,text=text)
   ...
   if text == 'مجموعة المطور' and m.chat.type == ChatType.PRIVATE:
     if not await dev_pls(m.from_user.id,m.chat.id):
        ...
        if not await r.get(f'DevGroup:{Dev_Zaid}'):
           return await m.reply(quote=True,text=f'{k} مجموعة المطور مو معينة')
        else:
           id = int(await r.get(f'DevGroup:{Dev_Zaid}'))
   ...
   if text == 'مسح مجموعة المطور':
     if not await devp_pls(m.from_user.id,m.chat.id):
        return
     await r.delete(f'DevGroup:{Dev_Zaid}')
   ...
   if text == 'تغيير المطور الاساسي':
     if not await devp_pls(m.from_user.id,m.chat.id):
        ...
        await r.set(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}',1,ex=600)
   ...
   if text == 'جلب نسخة القروبات' and await devp_pls(m.from_user.id,m.chat.id):
     list = []
     date = datetime.now()
     for chat in await r.smembers(f'enablelist:{Dev_Zaid}'):
   ...
   if text == 'جلب نسخة المستخدمين' and await devp_pls(m.from_user.id,m.chat.id):
     list = []
     date = datetime.now()
     for chat in await r.smembers(f'{Dev_Zaid}:UsersList'):
        list.append(int(chat))
     with open(f'{date}.json', 'w+') as w:
   ...
   if text == 'المكتومين عام':
      if not await dev_pls(m.from_user.id,m.chat.id):
        ...
        if not await r.smembers(f'listMUTE:{Dev_Zaid}'):
          ...
          text = '- المكتومين عام:\n\n'
          count = 1
          for PRE in await r.smembers(f'listMUTE:{Dev_Zaid}'):
             if count == 101: break
             try:
